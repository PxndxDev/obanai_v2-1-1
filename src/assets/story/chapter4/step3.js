...
const id = 3;
const name = "Une survie aisée";
const intrigue = "Cela fait plus de deux jours que vous survivez, et vous tenez debout !";

const exe = async (client, message, args) => {
    const s = new Step(client, message, args);
    const nar = require("../../narration/chapter4.json").step3;

    await s.createDialogue(null, null, { title: "UNE SURVIE AISÉE, partie 1", content: nar.dialog1 });

    let pDatas = await Players.get(client, message.author.id);
    const demon = demons_sauvages.get("Démon à Corne")[1];
