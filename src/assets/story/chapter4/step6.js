...
const id = 6;
const name = "C'est bientôt fini ?";
const intrigue = "Ça fait combien de temps ? Des semaines hein ? Non, des mois..";

const exe = async (client, message, args) => {
    const s = new Step(client, message, args);
    const nar = require("../../narration/chapter4.json").step6;

    await s.createDialogue(null, null, { title: "C'EST BIENTÔT FINI ? partie 1", content: nar.dialog1 });
