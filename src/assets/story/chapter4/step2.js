...
const id = 2;
const name = "Le premier jour";
const intrigue = "Voici votre premier jour de survie.";

const exe = async (client, message, args) => {
    const s = new Step(client, message, args);
    const nar = require("../../narration/chapter4.json").step2;

    await s.createDialogue(null, null, { title: "LE PREMIER JOUR, partie 1", content: nar.dialog1 });

    let pDatas = await Players.get(client, message.author.id);
    const demon = demons_sauvages.get("Démon aux Longs Membres")[1];
