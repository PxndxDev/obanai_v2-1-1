...
const id = 4;
const name = "Le début des ennuis";
const intrigue = "Cela fait 3 jours que vous êtes ici, et vous commencez à manquer de force, de plus les ennemis sont de plus en plus coriaces.";

const exe = async (client, message, args) => {
    const s = new Step(client, message, args);
    const nar = require("../../narration/chapter4.json").step4;

    await s.createDialogue(null, null, { title: "LE DÉBUT DES ENNUIS, partie 1", content: nar.dialog1 });

    let pDatas = await Players.get(client, message.author.id);
    const demon = demons_sauvages.get("Démon Supersonique")[1];
