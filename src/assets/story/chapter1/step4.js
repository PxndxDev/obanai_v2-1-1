...
const exe = async (client, message, args) => {
    ...
    await s.createDialogue(null, null, { "title": "PRESQUE RÉTABLIT, partie 1", "content": dialog1 });

    const results = [];

    if (loc === "Chemin étroit") {
        await s.createDialogue(null, null, { "title": "PRESQUE RÉTABLIT, partie 2", "content": nar.dialog2[0] });
        ...
        await s.createDialogue(null, null, { "title": "PRESQUE RÉTABLIT, partie 3", "content": nar.dialog2[1] });
        ...
        await s.createDialogue(null, null, { "title": "PRESQUE RÉTABLIT, partie 2", "content": nar.dialog2[2] });
        const r1 = await s.strengthMiniGame();
        await client.resp(message.author, message.channel, `Votre exercice est terminé. Vous avez obtenu un score de **${r1.results.score}**, et vous obtenez le rang **${r1.results.classe[0]}** !`, "normal");


        await s.createDialogue(null, null, { "title": "PRESQUE RÉTABLIT, partie 3", "content": nar.dialog[3] });
    ...
    await s.createDialogue(null, null, { "title": "PRESQUE RÉTABLIT, partie 4", "content": [
    ...
    await s.createDialogue(null, null, { "title": "PRESQUE RÉTABLIT, partie 5", "content": nar.dialog3 });
